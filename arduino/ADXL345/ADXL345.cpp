...
ADXL345::ADXL345() {
	...
	/* Specific calibration values in original code,
	base calibration should be here (256 LSB/g) -> .00390625 g/LSB
	gains[0] = 0.00376390;
	gains[1] = 0.00376009;
	gains[2] = 0.00349265;
	*/
...
void ADXL345::powerOn() {
	...
	// Why do all three writes? still not sure
...
// Reads raw acceleration into a vector
void ADXL345::readAccel(int *xyz){
	readAccel(xyz, xyz + 1, xyz + 2);
}

// Reads raw acceleration into 3 variables
...
// Reads scaled acceleration (in g) to a vector
...
// Reads scaled acceleration (in g) to three variables
void ADXL345::get_Gxyz(double *x, double *y, double *z){
	int xyz_int[3];
	readAccel(xyz_int);
	*x = ((((int)_buff[1]) << 8) | _buff[0])*gains[0];   
	*y = ((((int)_buff[3]) << 8) | _buff[2])*gains[1];
	*z = ((((int)_buff[5]) << 8) | _buff[4])*gains[2];
}
...
void ADXL345::getRangeSetting(byte* rangeSetting) {
	...
	// Range setting is contained in two LSB's
...
void ADXL345::setRangeSetting(int val) {
	...
	// get current value in DATA_FORMAT and OR it with range value
	readFrom(ADXL345_DATA_FORMAT, 1, &_b);
	_s |= (_b & B11101100);
...
// Set sampling rate - use value in Hz
/* Acceptable values:
3200
1600
800
...
// Set sampling rate using actual bandwidth code
...
// Get sampling rate bandwidth code
